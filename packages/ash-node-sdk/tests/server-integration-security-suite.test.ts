...
describe('API Quality (AQ)', () => {
  ...
  describe('AQ-003: Context Store TTL', () => {
    ...
    it('should handle cleanup', async () => {
      const store = new AshMemoryStore({ ttlSeconds: 1, cleanupIntervalSeconds: 1 });
      const ctx = createValidContext();
...
describe('Security Audit', () => {
  ...
  describe('SA-004: Memory Safety', () => {
    it('should zero out sensitive data on destroy', () => {
      ...
      // After destroy, the proof should be cleared
      // (internal implementation detail - just ensure no crash)
  ...
  describe('SA-006: Timing Attack Resistance', () => {
    it('should use constant-time comparison for body hash', () => {
...
describe('Fuzz Testing', () => {
  ...
  describe('FUZZ-002: Malformed Headers', () => {
    ...
    it('should accept whitespace-only context ID (valid chars)', () => {
      // Whitespace is not a control character, so it's technically accepted
      // The validation happens later in the flow
      const headers = createValidHeaders();
...
describe('Integration Tests', () => {
  ...
  describe('INT-003: Middleware Integration', () => {
    ...
    it('Fastify: full plugin flow with valid context', async () => {
      ...
      expect(reply.code).not.toHaveBeenCalledWith(expect.any(Number));
      expect(req.ash).toBeDefined();
    });
  });
});

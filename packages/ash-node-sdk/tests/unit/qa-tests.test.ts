...
describe('QA: SDK version and constants', () => {
  ...
  it('QA-CONST-001: All constants are exported and have correct types', () => {
    expect(typeof MAX_PAYLOAD_SIZE).toBe('number');
    expect(typeof MAX_RECURSION_DEPTH).toBe('number');
    expect(typeof MAX_SCOPE_FIELDS).toBe('number');
    expect(typeof MAX_NONCE_LENGTH).toBe('number');
    expect(typeof MIN_NONCE_HEX_CHARS).toBe('number');
    expect(typeof MAX_BINDING_LENGTH).toBe('number');
    expect(typeof MAX_CONTEXT_ID_LENGTH).toBe('number');
    expect(typeof MAX_QUERY_PARAMS).toBe('number');
    expect(typeof MAX_TIMESTAMP).toBe('number');
    expect(typeof SHA256_HEX_LENGTH).toBe('number');
    expect(typeof SCOPE_FIELD_DELIMITER).toBe('string');
    expect(typeof PIPE_DELIMITER).toBe('string');
    expect(typeof DEFAULT_MAX_TIMESTAMP_AGE_SECONDS).toBe('number');
    expect(typeof DEFAULT_CLOCK_SKEW_SECONDS).toBe('number');
  });

  it('QA-CONST-002: Constants have correct values', () => {
...
describe('QA: JSON canonicalization edge cases', () => {
  ...
  it('QA-JSON-019: String with only whitespace', () => {
    expect(ashCanonicalizeJson('"   "').length).toBeGreaterThan(0);
  });
...
describe('QA: End-to-end flow', () => {
  ...
  it('QA-E2E-006: Whitespace-only payload handled as empty', () => {
    const secret = ashDeriveClientSecret(NONCE, CTX, BINDING);
    const r = ashBuildProofUnified(secret, TS, BINDING, '   ', [], null);
    expect(typeof r.proof).toBe('string');

...
import { ashVerifyProofUnified } from '../../../src/proof-unified.js';
import { ashVerifyProofScoped } from '../../../src/proof-scoped.js';
...
describe('AQ: Build request — destroy', () => {
  it('AQ-BR-DEST-001: destroy is a function', () => {
    const result = ashBuildRequest(basicInput());
    expect(typeof result.destroy).toBe('function');
  });

  it('AQ-BR-DEST-002: destroy can be called without error', () => {
    const result = ashBuildRequest(basicInput());
    expect(() => result.destroy()).not.toThrow();
  });

  it('AQ-BR-DEST-003: destroy can be called multiple times', () => {
    const result = ashBuildRequest(basicInput());
    result.destroy();
    expect(() => result.destroy()).not.toThrow();
  });

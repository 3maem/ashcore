...
mod security_audit_tests {
    ...
    fn sa_memory_safety_zeroization() {
        ...
        let _secret = ash_derive_client_secret(nonce, context_id, binding).unwrap();

        // Proof building uses Zeroizing for message
        let body_hash = ash_hash_body(r#"{"a":1}"#);
        let _proof = ash_build_proof(&_secret, "1000000000", binding, &body_hash).unwrap();

        // If we reach here without panic, Zeroizing is being used
        assert!(true);
    }
...
mod fuzz_tests {
    ...
    fn fuzz_malformed_input() {
        ...
        for input in malformed {
            ...
            // Should fail gracefully, never panic
            assert!(
                result.is_err() || result.is_ok(),
                "Should not panic for: {}",
                input
            );
        }
    }

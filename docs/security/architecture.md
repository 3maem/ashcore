# ashcore Security Architecture

This document explains the system-level security architecture of ashcore and how its components interact to provide request integrity and replay protection.
...
ashcore operates as a request integrity library embedded in both client and server applications.
...
### Server Application (using ashcore)
...
### Client Application (using ashcore)
...
At the architecture level, ashcore guarantees:
...
| ashcore | Integrity + Replay protection |

ashcore complements -- not replaces -- these controls.
...
ashcore provides integrity verification for HTTP requests between client and server applications.
